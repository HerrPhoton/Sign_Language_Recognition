...
Данный датасет состоит из точек и их меток. Для точек есть три координаты, а также принадлежность к классу: лицо, тело, правая рука, левая рука. Чтобы работать с такими данными пришлось обработать фйалы 
особого расширения, где хранились кадры видео и положения точек в пространстве.
...
## Тестирование модели 
Наша функция теста выдает нам готовый data frame.
Для того чтобы это сделать используем несколько функций. 
+ В первую очередь мы записываем видео с веб-камеры и сохраняем в файл с помощью функции create_video().
+ Далее мы нарезаем видео по картинкам с помощью функции cut_video(), получается одна картинка каждые 10 кадров. 
+ Получаем путь к папке с картинками и создаем наш data frame с помощью функции create_test_df(), в которую мы передаем путь к нашему видео. Эта функция пробегается по папке с картинками и на каждой картинке отрисовывает скелет человека (точки лица, левой руки, правой руки и самого тела ) 
+ Затем функция берёт координаты точек и записывает их в списки, из которых мы создаем наши data frame(для каждой части скелета), далее конкатенирует все data frame и выдает нам готовую базу данных.
